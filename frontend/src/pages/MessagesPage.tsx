  ...
  return (
    <div className="min-h-screen bg-canvas pt-20 px-6 pb-10">
      <div className="max-w-sm mx-auto mt-4">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-fg-faint hover:text-fg text-sm mb-4 transition-colors"
        >
          <ArrowLeft size={16} weight="bold" /> Volver
        </button>
        <h1 className="text-2xl font-black text-fg mb-2">Mensajes</h1>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-fg-faint font-semibold">Sin conversaciones</p>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 rounded-2xl p-4 text-left transition-all border ${
                  c.unread > 0
                    ...
                    : 'bg-surface border-line hover:border-line-strong hover:bg-subtle'
                }`}
              >
                {/* Avatar con badge de no leídos */}
                <div className="relative w-12 h-12 rounded-full bg-subtle overflow-hidden flex-shrink-0 flex items-center justify-center">
                  {c.other.avatar
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-0.5">
                    <p className={`text-sm truncate ${c.unread > 0 ? 'text-fg font-bold' : 'text-fg font-semibold'}`}>
                      {c.other.name}
                    </p>
                    {c.last_message && (
                      <span className="text-fg-faint text-[10px] flex-shrink-0">
                        {formatTime(c.last_message.created_at)}
                      </span>
                    )}
                  </div>
                  <p className="text-fg-faint text-[11px] truncate flex items-center gap-1">
                    {c.status === 'in_progress' && <span className="inline-block w-1.5 h-1.5 bg-warn rounded-full animate-pulse" />}
                    {c.ride.origin} → {c.ride.destination}
                  </p>
                  {c.last_message ? (
                    <p className={`text-xs mt-1 truncate ${c.unread > 0 ? 'text-fg-muted font-medium' : 'text-fg-faint'}`}>
                      {c.last_message.mine && <span className="text-fg-faint">Tú: </span>}
                      {c.last_message.text}
                    </p>
                  ) : (
                    <p className="text-fg-faint text-xs mt-1 italic">Sin mensajes aún · Empieza la conversación</p>
                  )}
                </div>

                <CaretRight size={14} weight="bold" className="text-fg-faint flex-shrink-0" />
              </button>
            ))}
          </div>
        )}
