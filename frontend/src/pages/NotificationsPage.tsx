...
export default function NotificationsPage() {
  ...
  const icon = (type: NotifType) => {
    if (type === 'booking_accepted')   return <CheckCircle   size={22} weight="duotone" className="text-live"  />;
    if (type === 'booking_rejected')   return <XCircle       size={22} weight="duotone" className="text-danger"    />;
    if (type === 'booking_request')    return <Users         size={22} weight="duotone" className="text-info"   />;
    if (type === 'booking_started')    return <Play          size={22} weight="duotone" className="text-star" />;
    if (type === 'booking_completed')  return <Star          size={22} weight="duotone" className="text-info"   />;
    if (type === 'departure_reminder') return <Clock         size={22} weight="duotone" className="text-warn" />;
    if (type === 'new_message')        return <ChatCircle    size={22} weight="duotone" className="text-info"   />;
    if (type === 'passenger_ready')    return <CheckCircle   size={22} weight="duotone" className="text-live"  />;
    if (type === 'new_rating')         return <Star          size={22} weight="fill"    className="text-star" />;
    if (type === 'passenger_delay')    return <Clock         size={22} weight="duotone" className="text-warn" />;
    if (type === 'passenger_declined') return <XCircle       size={22} weight="duotone" className="text-danger"    />;
    return <WarningCircle size={22} weight="duotone" className="text-star" />;
  };
    ...
    <div className="min-h-screen bg-canvas pt-20 px-6 pb-10">
      <div className="max-w-sm mx-auto mt-4">
        <div className="flex items-center justify-between mb-4">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-fg-faint hover:text-fg transition-colors text-sm">
            <ArrowLeft size={16} weight="bold" /> Volver
          </button>
          {notifs.length > 0 && (
            <button onClick={deleteAll} className="text-fg-faint hover:text-danger text-xs transition-colors">
              Borrar todas
            </button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-fg-faint font-semibold">Sin notificaciones</p>
            <p className="text-fg-faint text-sm mt-1">Aquí verás actualizaciones de tus viajes</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className={`text-sm font-semibold ${!n.read ? 'text-fg' : 'text-fg-muted'}`}>{n.title}</p>
                      {!n.read && <span className="w-2 h-2 rounded-full bg-notify flex-shrink-0" />}
                    </div>
                    <p className="text-fg-faint text-xs mt-0.5 leading-relaxed">{n.message}</p>
                    <p className="text-fg-faint text-xs mt-1.5">{timeAgo(n.created_at)}</p>
                  </div>
                  <div className="flex flex-col items-end gap-2 flex-shrink-0">
                    <button
                      onClick={(e) => deleteOne(e, n.id)}
                      className="p-1 text-fg-faint hover:text-danger transition-colors rounded"
                      title="Eliminar"
                    >
                      <Trash size={14} weight="duotone" />
                    </button>
                    {clickable && <CaretRight size={14} weight="bold" className="text-fg-faint" />}
                  </div>
                </div>
              );
            })}
